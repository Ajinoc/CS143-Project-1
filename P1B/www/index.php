<!DOCTYPE html>
<head>
    <title>CS143 Project 1B - Movie Database -</title>
    <link rel="stylesheet" href="style.css" />
    <base href=""/>
<body>

    <h1>Kaitlyn Cason and Adam Jones's Movie Database</h1>
    <iframe src="I1.php" name="iframe" id="iframe" style="overflow: hidden; height: 100%; width: 100%; position: absolute;" align=right frameborder="0" height=100% width=80% margin-left:0></iframe>

    <div id="menu">

        <p><b> Add: </b></p>
        <ul>
            <li><a href="I1.php" target="iframe">Actor/Director Information</a></li>
            <li><a href="I2.php" target="iframe">Movie Information</a></li>
            <li><a href="I3.php" target="iframe">Movie Comment</a></li>
            <li><a href="I4.php" target="iframe">Actor to Movie Relation</a></li>
            <li><a href="I5.php" target="iframe">Director to Movie Relation</a></li>
        </ul>

        <p><b> Search: </b></p>
        <ul>
            <li><a href="S1.php" target="iframe">Search for Actor/Actress/Movie</a></li>
        </ul>

        <p><b> Browse: </b></p>
        <ul>
            <li><a href="B1.php?param1=52794" target="iframe">Browse Actor Information</a></li>
            <li><a href="B2.php?param1=2632" target="iframe">Browse Movie Information</a></li>
        </ul>

    </div>

</head>
</body>
</html>


